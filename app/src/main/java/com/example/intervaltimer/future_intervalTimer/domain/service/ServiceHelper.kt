...
object ServiceHelper {
    ...
    fun clickPendingIntent(context: Context): PendingIntent {
        val clickIntent = Intent(context, MainActivity::class.java).apply {
            putExtra(INTERVAL_TIMER_STATE, IntervalTimeState.Started.name)
        }
    ...
    fun stopPendingIntent(context: Context): PendingIntent {
        val stopIntent = Intent(context, IntervalTimeService::class.java).apply {
            putExtra(INTERVAL_TIMER_STATE, IntervalTimeState.Stopped.name)
        }
    ...
    fun cancelPendingIntent(context: Context): PendingIntent {
        val cancelIntent = Intent(context, IntervalTimeService::class.java).apply {
            putExtra(INTERVAL_TIMER_STATE, IntervalTimeState.Canceled.name)
        }
        return PendingIntent.getService(
